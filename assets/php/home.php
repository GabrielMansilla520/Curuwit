...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $contenido = isset($_POST['postContent']) ? $_POST['postContent'] : '';
    ...
    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] == UPLOAD_ERR_OK) {
        ...
        move_uploaded_file($_FILES['imagen']['tmp_name'], 'ruta/donde/almacenar/' . $imagen_nombre);
    }

    // Insertar la nueva publicación con contenido e imagen (si se proporciona)
...
<html lang="es">
  ...
  <body>
    ...
    <main class="d-flex align-items-center py-4 bg-body-tertiary">
        <div class="container">
            <!-- Espacio para hacer publicaciones -->
            <!-- Espacio para hacer publicaciones -->
<div class="mb-4">
    <h2 class="mb-3">Hacer una publicación</h2>
    <form action="procesar_publicacion.php" method="post" enctype="multipart/form-data">
        <div class="mb-3">
            <label for="postContent" class="form-label">Contenido de la publicación</label>
            <textarea class="form-control" id="postContent" name="postContent" rows="3" placeholder="Escribe tu publicación"></textarea>
        </div>
        <div class="mb-3">
        ...
        <button type="submit" class="btn btn-primary">Publicar</button>
            ...
            <div>
                <h2 class="mb-3">Publicaciones</h2>
                <?php
// Consulta para obtener todas las publicaciones con información de usuario
$sql = "SELECT p.*, u.nombre_usuario, u.id as usuario_id, r.contenido as respuesta_contenido, r.fecha_publicacion as respuesta_fecha
        FROM publicaciones p
        INNER JOIN usuarios u ON p.usuario_id = u.id
        LEFT JOIN publicaciones r ON p.id = r.respuesta_a
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        if (!empty($row['imagen'])) {
            $imagen_ruta = './imagenes_publicaciones/' . $row['imagen'];
            echo "<img src='{$imagen_ruta}' alt='Imagen de la publicación' class='img-fluid mb-2'>";
        }

        echo "<p class='card-text'>{$row['contenido']}</p>";
        echo "<p class='card-text'><small class='text-muted'>Publicado el {$row['fecha_publicacion']} por <strong><a href='perfil.php?usuario_id={$row['usuario_id']}'>{$row['nombre_usuario']}</a></strong></small></p>";

        // Mostrar información de la respuesta
        if (!empty($row['respuesta_contenido'])) {
            echo "<hr>";
            echo "<p class='card-text'>{$row['respuesta_contenido']}</p>";
            echo "<p class='card-text'><small class='text-muted'>Publicado el {$row['respuesta_fecha']} en respuesta a la publicación de <strong><a href='perfil.php?usuario_id={$row['usuario_id']}'>{$row['nombre_usuario']}</a></strong></small></p>";
        }
        // Formulario para responder a publicaciones
        echo "<div class='mb-3'>";
        echo "<button class='btn btn-secondary' onclick='toggleReplyForm({$row['id']})'>Responder</button>";
        echo "<form id='replyForm{$row['id']}' action='procesar_publicacion.php' method='post' style='display:none;'>";
        echo "<textarea class='form-control mt-3' id='postContent' name='postContent' rows='3' placeholder='Escribe tu respuesta'></textarea>";
        echo "<input type='hidden' name='respuesta_a' value='{$row['id']}'>";
        echo "<button type='submit' class='btn btn-primary mt-2'>Enviar</button>";
        echo "</form>";
        echo "</div>";

        // Botón "Ver respuestas" y enlace
        echo "<div class='mb-3'>";
        echo "<button class='btn btn-info' onclick='viewReplies({$row['id']})'>Ver respuestas</button>";
        echo "<a href='respuestas_publicacion.php?id={$row['id']}' id='repliesLink{$row['id']}' style='display:none;'>Ver respuestas</a>";
        echo "</div>";
...
<footer class="text-center text-white sticky-bottom custom-footer" style="background-color: #6d7fcc;">
...
function viewReplies(postId) {
    var repliesLink = document.getElementById('repliesLink' + postId);
    window.location.href = repliesLink.href;
}
</script>
