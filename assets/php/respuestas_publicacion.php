...
// Verificación de sesión y cookie
...
// Procesamiento de la publicación
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $contenido = isset($_POST['postContent']) ? $_POST['postContent'] : '';
    $respuesta_a = isset($_POST['respuesta_a']) ? $_POST['respuesta_a'] : null; // ID de la publicación a la que se responde

    // Inserta la nueva publicación
    $sql = "INSERT INTO publicaciones (usuario_id, contenido, respuesta_a) VALUES (?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("iss", $_SESSION['usuario_id'], $contenido, $respuesta_a);
    $stmt->execute();
...
<div>
    ...
    <h2 class="mb-3">Publicación</h2><?php
    // respuestas_publicacion.php

    include('conexion.php');

    if (isset($_GET['id'])) {
        $publicacion_id = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT);

        // Consulta para obtener la publicación original y sus respuestas
        $sql = "SELECT p.*, u.nombre_usuario AS autor
                FROM publicaciones p
                INNER JOIN usuarios u ON p.usuario_id = u.id
                WHERE p.id = ? OR p.respuesta_a = ?
                ORDER BY p.fecha_publicacion ASC";

        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ii", $publicacion_id, $publicacion_id);
        $stmt->execute();
        $result = $stmt->get_result();

        // Mostrar publicación original y respuestas
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<div class='card mb-3'>";
                echo "<div class='card-body'>";
                echo "<p class='card-text'>{$row['contenido']}</p>"; 
                echo "<p class='card-text'><small class='text-muted'>Publicado el {$row['fecha_publicacion']} por <strong>{$row['autor']}</strong></small></p>";
            ...
            echo "No hay publicación para esta ID.";
        }
