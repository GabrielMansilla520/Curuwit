...
<html lang="es"  data-bs-theme="auto">
  ...
  <body>
    ...
    if ($usuario) {
        ...
        echo "<h2 class='text-center'>Perfil de {$usuario['nombre_usuario']}</h2>";
        echo "<img class='fotos_perfil' src='" . $usuario['foto_perfil'] . "' alt='Foto de Perfil'>";
        echo "<p>Nombre: <strong>{$usuario['nombre']}</strong></p>";
        echo "<p>Apellido: <strong>{$usuario['apellido']}</strong></p>";
        echo "<p>Fecha de Nacimiento: <strong>{$usuario['fecha_nacimiento']}</strong></p>";

        // Mostrar las publicaciones del usuario
        if (!empty($usuario['id'])) {
            echo "<h2>Publicaciones del usuario</h2>";
            if (!empty($usuario['contenido'])) {
                do {
                    echo "<div class='card mb-3'>";
                    echo "<div class='card-body'>";
                    echo "<p class='card-text'>{$usuario['contenido']}</p>";
                    echo "<p class='card-text'><small class='text-muted'>Publicado el {$usuario['fecha_publicacion']}</small></p>";
                    echo "</div>";
                    echo "</div>";
                } while ($usuario = $result->fetch_assoc());
            } else {
                echo "El usuario aún no ha realizado ninguna publicación.";
            }
        }
    } else {
